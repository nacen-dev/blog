...
describe("BlogPosts", async () => {
  ...
  it("should render no blog post found when there are no blog posts", () => {
    const { getByText } = render(<BlogPosts blogPosts={[]} />);
    expect(getByText(/no blog post found/i)).toBeInTheDocument();
  });

  it("should render a list of blog post that have published at and title", () => {
    const { getAllByText } = render(<BlogPosts blogPosts={testDataList} />);

    for (const testData of testDataList) {
      const title = getAllByText(testData.metadata.title);
      expect(title[0]).toBeInTheDocument();

      const publishedAt = getAllByText(
  ...
  it("should change color when a blog post title is hovered and returns to previous color when unhovered", async () => {
    ...
    const hoveredTitle = getAllByText(testDataList[0].metadata.title)[0];
